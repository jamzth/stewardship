...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		$filter_boxes = $field->options( 'filter_boxes' )
			? '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Find %s', 'cmb' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>'
			: '';
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'All %s', 'cmb' ), $post_type_labels ) . '</h4>';
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Family Members', 'cmb' ), $post_type_labels ) . '</h4>';
		$filter_boxes = $field->options( 'filter_boxes' )
			? '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Find Family Members', 'cmb' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>'
			: '';
